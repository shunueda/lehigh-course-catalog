...
browsers.forEach(async (browser, i) => {
    ...
    for (const courseNumber of chunks[i]) {
        ...
        await page.goto("https://reg-prod.ec.lehigh.edu/StudentRegistrationSsb/ssb/term/termSelection?mode=search", {
        ...
        await subPage.goto("https://reg-prod.ec.lehigh.edu/StudentRegistrationSsb/ssb/searchResults/searchResults?pageMaxSize=999", {
            waitUntil: "networkidle2",
        });
        const extracted = await subPage.$eval("*", (el) => el.innerText);
        await subPage.close();
        await page.close();
        writeFileSync(`./assets/chunks/${i}_${++chunkIndex}.json`, JSON.stringify(JSON.parse(extracted).data));
    }
    status[i] = true;
    if (status.every(Boolean)) {
        ...
        for (const chunk of readdirSync("assets/chunks")) {
            courses.push(...JSON.parse(readFileSync(`assets/chunks/${chunk}`).toString()));
        }
        writeFileSync("assets/courses.json", JSON.stringify(_.uniqWith(courses, _.isEqual)));
